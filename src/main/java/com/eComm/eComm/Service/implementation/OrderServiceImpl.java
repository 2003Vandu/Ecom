...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional //CRITICAL: Ensures order creation + stock reduction is atomic
    public OrderResponse createOrder(OrderRequest request) {
        ...
        for (OrderRequest.OrderItemRequest item : request.getCartItems()) {
            boolean isAvailable = inventoryService.checkStockAvailability(
                    item.getItemId(),
                    item.getQuantity()
            );
        ...
        try {
            ...
            // If stock reduction fails, transaction will rollback
        ...
        log.info("Order created successfully: {} | Total items: {}",
                newOrder.getOrderId(), orderItems.size());

        return convertToResponse(newOrder);
    }

    @Override
    @Transactional //  Delete order should restore stock
    public void deleteOrder(String orderId) {
        OrderEnitity existingOrder = orderEntityRepository.findByOrderId(orderId)
        ...
        for (OrderItemEntity item : existingOrder.getItems()) {
            try {
                ...
                log.warn("Could not restore stock for item: {}", item.getItemId());
            }
        }

        orderEntityRepository.delete(existingOrder);
        log.info("Order deleted and stock restored: {}", orderId);
    }
